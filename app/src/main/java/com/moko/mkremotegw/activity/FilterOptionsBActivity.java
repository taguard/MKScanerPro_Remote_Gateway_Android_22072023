...
import com.moko.mkremotegw.databinding.ActivityFilterConditionBinding;
...
public class FilterOptionsBActivity extends BaseActivity<ActivityFilterConditionBinding> implements SeekBar.OnSeekBarChangeListener {
    ...
    public void onMQTTMessageArrivedEvent(MQTTMessageArrivedEvent event) {
        ...
        if (msg_id == MQTTConstants.READ_MSG_ID_FILTER_B) {
            ...
            if (filterRawDatas.size() > 0) {
                for (int i = 0, l = filterRawDatas.size(); i < l; i++) {
                    ...
                    View v = LayoutInflater.from(FilterOptionsBActivity.this).inflate(R.layout.item_raw_data_filter, mBind.llRawDataFilter, false);
        ...
        if (msg_id == MQTTConstants.CONFIG_MSG_ID_FILTER_B) {
    ...
    private void getFilterConditionB() {
        String appTopic;
        ...
        String message = MQTTMessageAssembler.assembleReadFilterB(deviceInfo);
        try {
            MQTTSupport.getInstance().publish(appTopic, message, MQTTConstants.READ_MSG_ID_FILTER_B, appMqttConfig.qos);
    ...
    private void saveParams() {
        ...
        String message = MQTTMessageAssembler.assembleWriteFilterB(deviceInfo, filterCondition);
        try {
            MQTTSupport.getInstance().publish(appTopic, message, MQTTConstants.CONFIG_MSG_ID_FILTER_B, appMqttConfig.qos);
        } catch (MqttException e) {
            e.printStackTrace();
        }
    }
